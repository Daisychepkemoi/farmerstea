...
class TeaController extends Controller
{
    ...
    public function admindash()
    {
        ...
        if ($user->role == 'admin') {
            ...
            $rejected = User::where('verifiedadmin', 'denied')->where('role', 'user')->count();
            ...
            $revoked = User::where('verifiedadmin', 'revoked')->where('role', 'user')->count();
            $verifiedfarmer = new ProduceChart;
            $verifiedfarmer
            ->labels(['rejected','verified','unverified','revoked'])->options([
                    'legend' => ['display' => true],
                    ])
            ->title('Ratio Of Farmer registration')
            // ->elementLabel('farmers')
            // ->elementLabel("Total")
            ...
            $netmonth=Tea_Details::whereYear('date_offered', now())->select($netpermonth, $monthinyear)->groupby('month')->orderBy($order, 'asc')->pluck('month');
            $month=Tea_Details::whereYear('date_offered', now())->select($netpermonth, $monthinyear)->groupby('month')->orderBy($order, 'asc')->pluck('net');
            // total per month chart bar graph
            $totalpermonth = new SampleCharts;
            $totalpermonth->labels($netmonth->values())->options([
                    'legend' => ['display' => true],
                    ])->dataset('Total Monthly Produce Per Year', 'bar', $month->values())->options([
                    'label' => '# of Votes',
                    'borderColor' => '#ff0000',
                    'borderWidth' => 1,
                    'backgroundColor' => [
            ...
            $day=DB::raw('date(date_offered) as date');
    ...
    public function netperday()
    {
        ...
        $verifiedfarmer->labels(['rejected','verified','unverified','revoked'])->options([
                    ...
                    ->dataset('Farmer Verification', 'pie', [$rejected,$verified,$unverified,$revoked])
                    ->options([
                    'label' => '# of Votes',
                    'borderColor' => '#ff0000',
                    'borderWidth' => 1,
    ...
    public function netpermonth()
    {
        ...
        //no of farmers as per verification
        $rejected = User::where('verifiedadmin', 'denied')->count();
        $verified = User::where('verifiedadmin', 'verified')->count();
        $unverified = User::where('verifiedadmin', 'notverified')->count();
        ...
        // // foreach ($netmonth as $key ) {
        //   $m = date('F',mktime(0,0,0,$netmonth->values(),10));
        // // }
        // dd($m);  # code...
